...
class BalanceControllerNode(Node):
    """Filter roll/pitch and call high-level SDK balance actions for demo compensation."""

    def __init__(self, config_file: str, grpc_addr: str | None):
        super().__init__("dobot_balance_controller_node")
        ...
        self._enter_balance_stand = bool(robot_cfg.get("enter_balance_stand_on_start", True))
        ...
        self._connect_robot()
        self.get_logger().info("姿态补偿控制节点已启动，gRPC: %s" % self._grpc_addr)
        self.get_logger().info(
            "配置: execute=%s | mode=%s | trigger=%.2fdeg | limit=%.2fdeg | duration=%.2fs"
            % (self._enable_execute, self._mode, self._trigger_threshold, self._max_compensation, self._action_duration)
        )
        self.get_logger().info(
            "验收观察字段: [CTRL] raw=原始姿态 filtered=滤波姿态 error=姿态偏差 cmd=补偿输出"
        )

    def _connect_robot(self):
        if not self._enable_execute:
            self._publish_state("dry-run mode: execute disabled")
            return
        try:
            ...
            if self._enter_balance_stand:
                res = self._robot.balance_stand()
                if res is not None and getattr(res, "success", False):
                    self._publish_state("已切换到 BALANCE_STAND")
                else:
                    self._publish_state("切换 BALANCE_STAND 未确认成功")
        except Exception as exc:
            self._robot = None
            self._publish_state("连接机器人失败: %s；仅发布控制计算，不执行补偿动作" % str(exc))
    ...
    def _filter(self, raw_roll: float, raw_pitch: float) -> tuple[float, float]:
        ...
        if not self._filter_initialized:
            ...
            self._state.filtered_roll_deg = (
                self._alpha * avg_roll + (1.0 - self._alpha) * self._state.filtered_roll_deg
            )
            self._state.filtered_pitch_deg = (
                self._alpha * avg_pitch + (1.0 - self._alpha) * self._state.filtered_pitch_deg
            )
    ...
    def _execute_compensation(self, roll_cmd: float, pitch_cmd: float):
        with self._action_lock:
            ...
            self._publish_state(text)
            if not self._enable_execute or self._robot is None:
                return
            try:
                if self._mode == "axis":
                    if abs(roll_cmd) >= self._settle_threshold:
                        self._robot.balance_roll(roll_cmd, self._action_duration, "dynamic", show_progress=False)
                    if abs(pitch_cmd) >= self._settle_threshold:
                        self._robot.balance_pitch(pitch_cmd, self._action_duration, "dynamic", show_progress=False)
                else:
                    duration = max(1.0, self._action_duration)
                    self._robot.dynamic_pose(
                        duration=duration,
                        roll_deg=roll_cmd,
                        pitch_deg=pitch_cmd,
                        yaw_deg=0.0,
                ...
                self.get_logger().error("姿态补偿动作执行失败: %s" % str(exc))
...
def main(argv=None):
    ...
    parser.add_argument("--config", required=True, help="balance_control_config.yaml 配置文件路径 / Path to balance_control_config.yaml")
    ...
    rclpy.init()
    node = BalanceControllerNode(args.config, args.grpc_addr)
    try:
        rclpy.spin(node)
